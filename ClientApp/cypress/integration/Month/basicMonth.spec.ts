...
describe('Month', () => {
  ...
  it('should add income', () => {
    cy.dayTotal('Dec 1, 2022').then( (beforeAmount) => {
      cy.getDay('Dec 1, 2022').find('.divButton').contains('Add Income').click();
      cy.transactionFill('test1', 7);
      cy.dayTotal('Dec 1, 2022').should('eq', beforeAmount + 7)
    })
    cy.getDay('Dec 1, 2022')
      .find('#incomes')
      .children()
      .contains('test1');
